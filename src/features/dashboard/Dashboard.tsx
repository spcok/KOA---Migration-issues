              ...
              {(filteredAnimals || []).map(animal => {
                ...
                return (
                  <tr key={animal.id} className="hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => onSelectAnimal(animal)}>
                    <td className="px-6 py-4 font-semibold text-slate-900">{animal.name}</td>
                    <td className="px-6 py-4 text-slate-500">{animal.species}</td>
                    <td className="px-6 py-4 text-slate-400">{animal.ring_number || '-'}</td>
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 rounded-md text-xs font-medium">active</span>
                    </td>
                    <td className="px-6 py-4 text-slate-400">{d?.todayWeight ? getWeightDisplay(d.todayWeight, animal.weight_unit) : '-'}</td>
                    <td className="px-6 py-4 text-slate-400">{d?.previousWeight ? getWeightDisplay(d.previousWeight, animal.weight_unit) : '-'}</td>
                    <td className="px-6 py-4 text-slate-400">{animal.flying_weight_g ? formatWeightDisplay(animal.flying_weight_g, animal.weight_unit) : '-'}</td>
                    <td className="px-6 py-4 text-slate-400">{d?.todayFeed ? (typeof d.todayFeed.value === 'string' ? d.todayFeed.value : String(d.todayFeed.value || 'Fed')) : '-'}</td>
                    <td className="px-6 py-4 text-blue-500">{animal.location}</td>

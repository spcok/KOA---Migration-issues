...
export function useMissingRecordsData() {
  const alerts = useLiveQuery(async () => {
    ...
    for (const animal of activeAnimals) {
      ...
      const lastWeightLog = await db.logEntries
        .where('animal_id')
        .equals(animal.id)
      ...
      const lastFeedLog = await db.logEntries
        .where('animal_id')
        .equals(animal.id)
  ...
  const husbandryStatus = useLiveQuery(async () => {
    ...
    for (const animal of activeAnimals) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const logs = await db.logEntries
          .where('animal_id')
          .equals(animal.id)
          .filter(log => log.log_date.startsWith(dateStr))
          .toArray();

        weights[6 - i] = logs.some(l => l.log_type === LogType.WEIGHT);
        feeds[6 - i] = logs.some(l => l.log_type === LogType.FEED);
      }

...
import { Plus, Edit2, Trash2, Shield, User as UserIcon, Lock, Users, Check, X as CloseIcon } from 'lucide-react';
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b-2 border-slate-100">
                  <th className="px-8 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest">Staff Member</th>
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">ID: {user.id.slice(0, 8)}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
        ...
        <div className="bg-white rounded-[2rem] border-2 border-slate-200 shadow-sm overflow-hidden animate-in fade-in duration-300">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b-2 border-slate-100">
                  <th className="px-8 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest min-w-[200px]">Permission Flag</th>
                  {configurableRoles.map(role => (
                    <th key={role} className="px-8 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">
                      {role.replace('_', ' ')}
                    </th>
                  ))}
                </tr>
                ...
                {permissionGroups.map(group => (
                  <React.Fragment key={group.title}>
                    <tr className="bg-slate-100/50">
                      <td colSpan={configurableRoles.length + 1} className="px-8 py-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        --- {group.title} ---
                      </td>
                    </tr>
                    {group.fields.map(field => (
                      <tr key={field.key} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        {configurableRoles.map(role => {
                          ...
                          const isChecked = config ? !!(config as any)[field.key] : false;
                          return (
                            <td key={role} className="px-8 py-5 text-center">
                              <button
                                onClick={() => updateRolePermission(role, field.key as any, !isChecked)}
                              ...
                              >
                                {isChecked ? <Check size={20} strokeWidth={3} /> : <CloseIcon size={16} />}
                              </button>
                            </td>
                          );
                        })}

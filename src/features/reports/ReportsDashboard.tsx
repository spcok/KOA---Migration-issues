                  ...
                  <thead>
                    <tr className="bg-slate-50/30">
                      {activeReport.columns.map((col, idx) => (
                        <th key={idx} className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] border-b border-slate-100">
                          {col}
                        </th>
                      ))}
                    </tr>
                    ...
                    {previewData.map((row, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                        {Object.values(row).map((val: string, vIdx) => (
                          <td key={vIdx} className={`px-10 py-6 text-sm ${vIdx === 0 ? 'font-bold text-slate-800' : 'text-slate-500'} ${vIdx === activeReport.columns.length - 1 ? 'font-black text-slate-400' : ''}`}>
                            {val}
                          </td>
                        ))}
                      </tr>
